...
.. note:: Note we convert from ``[start, end]`` to ``[start-1, start)``. The ``vcf2bed`` script performs the equivalent of a *single-base insertion* to make BED output that is guaranteed to work with BEDOPS, regardless of what the actual variant may be. The converted output contains additional columns which allow reconstruction of the original VCF data.
...
The ``vcf2bed`` script requires Python, version 2.7 or greater.
