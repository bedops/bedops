...
1. Review the `Github issues list <https://github.com/bedops/bedops/issues>`_
   ...
   b. If any issues can't be closed out, rename the assigned version tag to the next anticipated release version (*e.g.*, *v2p4p13* to *v2p5p0*, etc.)
...
2. Pull the most recent commit for the development branch to a local folder on build hosts (Linux with sufficiently old kernel, current OS X, etc.).
   ...
   The *X.Y.Z* scheme should follow the development branch name, *e.g.* 2.4.13, etc.
...
1. Merge BEDOPS development branch into master branch:
   ...
   As before, the *X.Y.Z* scheme should follow the development branch name, *e.g.* 2.4.13, etc.
