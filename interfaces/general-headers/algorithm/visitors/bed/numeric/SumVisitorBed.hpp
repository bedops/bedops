...
#ifndef CLASS_WINDOW_SUM_VISITOR_BED_H
#define CLASS_WINDOW_SUM_VISITOR_BED_H
...
namespace Visitors {
  ...
  namespace BedSpecific {
    ...
    struct Sum : public BaseVisitor {
      ...
      explicit Sum(const ProcessType& pt = ProcessType(), std::size_t col = 5)
          : pt_(pt), sum_(0), counter_(0), col_(col)
        { /* */ }

      inline void Add(MapType* bt)
        { sum_ += bt->measure(col_); ++counter_; }
...
#endif // CLASS_WINDOW_SUM_VISITOR_BED_H
