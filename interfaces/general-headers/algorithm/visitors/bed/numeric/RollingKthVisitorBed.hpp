...
#ifndef ROLLING_KTH_VISITOR_BED_HPP
#define ROLLING_KTH_VISITOR_BED_HPP
...
namespace Visitors {
  ...
  namespace BedSpecific {
    ...
    struct RollingKth : BaseVisitor {
      ...
      // Construction
      //==============
      explicit RollingKth(double kth = 0.8, const ProcessType& pt = ProcessType(), std::size_t col = 5)
          : kthValue_(kth), currentAtPos_(0), pt_(pt), col_(col), scoresBuf_(Comp(col)) {
      ...
      inline void DoneReference() {
        ...
        if ( size )
          pt_.operator()(static_cast<MT>((*currentMarker_)->measure(col_)));
        else {
...
#endif // ROLLING_KTH_VISITOR_BED_HPP
