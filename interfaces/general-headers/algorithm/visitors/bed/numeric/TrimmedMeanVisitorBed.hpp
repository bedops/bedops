...
#ifndef TMEANS_VISITOR_BED_HPP
#define TMEANS_VISITOR_BED_HPP
...
namespace Visitors {
  ...
  namespace BedSpecific {
    ...
    struct TrimmedMean : BaseVisitor {
      ...
      typedef Bed::CompValueThenAddressLesser Comp;
      typedef std::set<PtrType, Comp> ScoreTypeContainer;

      //==============
      // Construction
      //==============
      explicit TrimmedMean(double lowerKth=0.2, double upperKth = 0.8, const ProcessType& pt = ProcessType(), std::size_t col = 5)
          : lowerKth_(lowerKth), upperKth_(upperKth), lowerSum_(0), upperSum_(0), currentAtPosLower_(0),
            currentAtPosUpper_(0), doKth_(false), symmetric_(false), pt_(pt), col_(col), scoresBuf_(Comp(col)) {
