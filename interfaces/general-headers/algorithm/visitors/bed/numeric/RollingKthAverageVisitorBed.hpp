...
#ifndef ROLLING_KTH_VISITOR_AVERAGE_BED_HPP
#define ROLLING_KTH_VISITOR_AVERAGE_BED_HPP
...
#include "algorithm/visitors/bed/numeric/RollingKthVisitorBed.hpp"
...
namespace Visitors {
  ...
  namespace BedSpecific {
    ...
    struct RollingKthAverage : RollingKth<Visitors::Helpers::DoNothing, BaseVisitor, ExceptionType> {
      ...
      // Construction
      //==============
      explicit RollingKthAverage(double kth = 0.8,
                                 const ProcessType& pt = ProcessType(),
                                 std::size_t col = 5)
          : BaseClass(kth, Visitors::Helpers::DoNothing(), col), pt_(pt) { /* */ }
...
#endif // ROLLING_KTH_VISITOR_AVERAGE_BED_HPP
