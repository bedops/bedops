...
#ifndef _MAD_VISITOR_BED_HPP
#define _MAD_VISITOR_BED_HPP
...
#include "algorithm/visitors/bed/numeric/RollingKthAverageVisitorBed.hpp"
...
namespace Visitors {
  ...
  namespace BedSpecific {
    ...
    struct MedianAbsoluteDeviation : RollingKthAverage<
      ...
      explicit MedianAbsoluteDeviation(const ProcessType& pt = ProcessType(), double mult = 1, std::size_t col = 5)
         : BaseClass(0.5, Helpers::Keep<MT>(), col), pt_(pt), mult_(mult), col_(col)
        { /* */ }
      ...
      inline void DoneReference() {
        ...
        for ( IterType i = BaseClass::scoresBuf_.begin(); i != BaseClass::scoresBuf_.end(); ++i )
          vec.push_back(static_cast<MT>((*i)->measure(col_)));
