...
set bam2starch_script = "/net/lebowski/vol1/sw/bedops/2.4.3/bin/bam2starch"
set starchcat_bin = "/net/lebowski/vol1/sw/bedops/2.4.3/bin/starchcat"
...
foreach chrom (`$samtools_bin idxstats $input | cut -f1`)
  qsub $sge_opts -N $nm.$cntr > /dev/stderr << __EXTRACTION__
    $samtools_bin view -b $input $chrom | $python_bin $bam2starch_script > $here/$nm/$cntr
__EXTRACTION__
