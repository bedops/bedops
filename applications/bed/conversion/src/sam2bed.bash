...
#
# Project:      Convert SAM to UCSC BED
#
# Version:      2.3
#
...
printUsage () {
    cat <<EOF
Usage:
  sam2bed [ --help ] [ --do-not-sort | --max-mem <value> ] < foo.sam
        ...
        -)
            case "${OPTARG}" in
                ...
                do-not-sort)
                    ...
                    ;;
                max-mem)
                    echo "[sam2bed] - Warning: --max-mem option currently disabled" >&2
                    val="${!OPTIND}"; 
                    OPTIND=$(( $OPTIND + 1 ));
                    if [[ ! ${val} ]]; then
                        echo "[sam2bed] - Error: Must specify value for --max-mem" >&2
                        printUsage;
                        exit -1;
                    fi
                    maxMem="${val}";
