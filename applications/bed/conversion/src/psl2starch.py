...
def printUsage(stream):
    usage = ("Usage:\n"
             "  %s [ --help ] [ --keep-header ] [ --split ] [ --headered ] [ --do-not-sort | --max-mem <value> (--sort-tmpdir <dir>) ] [ --starch-format <bzip2|gzip> ] < foo.psl > sorted-foo.psl.bed.starch\n\n"
             "Version:\n"
             "  v2.4.3\n\n"
             "Options:\n"
...
def main(*args):
    ...
    optstr = ""
    longopts = ["do-not-sort", "keep-header", "split", "headered", "help", "max-mem=", "starch-format=", "sort-tmpdir="]
    try:
