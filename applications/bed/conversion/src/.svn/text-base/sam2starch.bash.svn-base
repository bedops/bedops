...
#               This output is passed to BEDOPS starch to create the archive.
#
...
#               Example usage:
#
#               $ sam2bed < foo.sam > sorted-foo.sam.bed.starch
#
...
#
#               $ sam2bed --do-not-sort < foo.sam > unsorted-foo.sam.bed.starch
#

printUsage () {
    cat <<EOF
Usage:
  sam2starch [ --help ] [  --do-not-sort | --max-mem <value> ] [ --starch-format <bzip2|gzip> ] < foo.sam > sorted-foo.sam.bed.starch
...
About:
  This script converts SAM data to lexicographically-sorted BED. The SAM (Sequence Alignment/Map) 
  format is 1-based, closed [a,b]. We convert this indexing back to 0-based, half-open [a-1,b) 
  when creating BED output. This output is passed to BEDOPS starch to create the archive.
