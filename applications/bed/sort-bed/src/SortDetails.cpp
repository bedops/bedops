...
Bed::LineCountType
appendChromBedEntry(ChromBedData *chrom, Bed::CoordType startPos, Bed::CoordType endPos, char *data, double *bytes)
{
  ...
  if(((index + 1) % NUM_BED_ITEMS_EST) == 0)
    {
      ...
      chrom->coords = (BedCoordData*)realloc(chrom->coords, sizeof(BedCoordData) * (index + NUM_BED_ITEMS_EST));
...
int
checkfiles(const char **bedFileNames, int numFiles)
{
  FILE* bedFile;
  int notStdin = 0, stdinCount = 0, i = 0;
...
int
mergeSort(FILE **tmpFiles, int numFiles)
{
  /* error checking in processData() has already been performed, headers and empty rows removed, etc. */
  int i = 0, done = 0, currMin = 0, val = 0;
  char **chroms = (char**)malloc(sizeof(char*) * numFiles);
  if(chroms == NULL)
    return -1;

  Bed::SignedCoordType *starts = (Bed::SignedCoordType*)malloc(sizeof(Bed::SignedCoordType) * numFiles);
  if(starts == NULL)
    return -1;

  Bed::SignedCoordType *ends = (Bed::SignedCoordType*)malloc(sizeof(Bed::SignedCoordType) * numFiles);
  if(ends == NULL)
    return -1;

  int *fields = (int*)malloc(sizeof(int) * numFiles);
  if(fields == NULL)
    return -1;

  char **rests = (char**)malloc(sizeof(char*) * numFiles);
  if(rests == NULL)
    return -1;
...
int
processData(const char **bedFileNames, int numFiles, double maxMem)
{
  /* maxMem will be ignored if <= 0 */

  FILE *bedFile;
  int i, j, k, notStdin = 0, chromEntryCount, newChrom,
    fields = 0, chromAllocs = 1,
    tmpFileCount = 0, headCheck = 1;
      ...
      while(fgets(bedLine, BED_LINE_LEN+1, bedFile))
        {
          ...
          if(cptr - bedLine > (double)CHROM_NAME_LEN)
            {
          ...
          memcpy(chromBuf, bedLine, cptr-bedLine);
